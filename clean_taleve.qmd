...
-   Année a un effet positif significatif sur l’abondance des talèves. L’effet de l’assec sur l’abondance est négatif (β = `r assec_res$beta`, p = `r assec_res$p`). Donc nous avons une augmentation de l'abondance de talèves au cours des années. L’assec partiel a un effet négatif significatif sur l’abondance des talèves. Le modele nous donne une augmentaion par année d'environ `r exp(annee_res$beta)`de talèves dans la reserve independamment des facteurs etudiés dans le modèle
...
-   L’effet de l’assec sur l’abondance est négatif (β = `r AssecT_1total_res$beta`, p = `r AssecT_1total_res$p`). Donc les evenements d'assec ont un impact négatif qui fait diminuer la popualtion de taleves d'environ `r exp(assec_res$beta)` par rapport à l'année précedente.
...
```{r}
p2 = plot(ggeffect(modele_complet, terms= "AssecT_1", residual = T))
p2
```
