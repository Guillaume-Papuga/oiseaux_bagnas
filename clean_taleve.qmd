...
setwd("data/pro")
process_data <- read_csv("process_data.csv")
...
# =========================
# plot des effets du meilleur MODELE
# =========================
mod <- glm(
  Occurence ~ annee + julian + I(julian^2) + nom_lieu +
    pluie_moy_glissante_7j + vent_moyen,
  family = binomial,
  data = Protocole_SuiviTaleve)
...
# EFFET DE L'ANNEE
# =========================
pred_annee <- ggpredict(mod, terms = "annee")

p1 <- ggplot(pred_annee, aes(x = x, y = predicted)) +
  geom_line(linewidth = 1) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.2) +
  labs(
    x = "Année",
    y = "Probabilité de présence",
    title = "Effet de l'année"
  ) +
  theme_classic()
...
# EFFET DE LA PLUIE
# =========================
pred_pluie <- ggpredict(mod, terms = "pluie_moy_glissante_7j")

p2 <- ggplot(pred_pluie, aes(x = x, y = predicted)) +
  geom_line(linewidth = 1) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.2) +
  labs(
    x = "Pluie moyenne glissante (7 jours)",
    y = "Probabilité de présence",
    title = "Effet de la pluie"
  ) +
  theme_classic()
...
# EFFET DU LIEU
# =========================
pred_lieu <- ggpredict(mod, terms = "nom_lieu")

p3 <- ggplot(pred_lieu, aes(x = reorder(x, predicted), y = predicted)) +
  geom_point(size = 3) +
  geom_errorbar(
