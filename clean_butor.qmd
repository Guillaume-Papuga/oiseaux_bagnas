...
##### création des lignes pour les jour  manquants du tableau 
...
butor_daily <- data_butor

data_butor$Date <- data_butor$date_debut
...
#### c - Cumule par jour julien au sein de chaque année
...
# boucle qui permet d'obtenir 1 graph par année 
...
#### VERIFER QU'IL Y A PAs DE DUPLICATION DE site comme Raph en avait trouvé

#### d - Cumule par année pour chaque site (même les sites hors protocole)

```{r}

# sommer les obsvertions par site et par année
obs_site <- butor_daily %>%
  group_by(annee, nom_lieu) %>%
  summarize(sum(nombre_min, na.rm = TRUE))

# boucle qui permet d'obtenir 1 graph par site 
