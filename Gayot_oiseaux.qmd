...
```
-------------------------------------------------
# 1.Netoyage de données
-------------------------------------------------
Nous allons mettre au propre le tableau 1 et le tableau 2 afin de vérifier s’il y a des différences entre les deux tableaux, corriger ces différences, et créer un nouveau tableau qui sera composé des trois espèces.
...
```
###############################################
###Mise au propre du tableau 1 
###############################################

Nous gardons seulement les colonnes qui nous semblent pertinentes pour la suite de l'analyse
...
###############################################
### Import et sélection du second tableau brut
###############################################
...
###############################################
###Vérification que les données sensibles correspondent
###############################################

Nous allons vérifier, après modification de la nature de certaines colonnes, si les jeux de données sont maintenant égaux. Nous les testerons espèce par espèce.
...
###############################################
###Ajout de la taleve sultane au tableau final
###############################################
...
-------------------------------------------------
# 1.Jeux de données nécessaires (par espece, par protocole...)
-------------------------------------------------
...
- blongios/taleve :  petit_bassin R3.2
              petit_bassin R2.9
              petit_bassin R3.1
              GB6
              chemin_animation
...
- butor :     GB
              ...
              99
              33
              domaineGrandClavalet
              
sites avec protocole régulier : 
- blongios/taleve : début en 2014 
- butor : début en 2013 
...
````
**Interpretation**
- tout confondu : max au  crépuscule (20h) et au levé du soleil (4-5h) 
...
**Interpretation** 
3 sites sans noms 
3 sites avec des noms différents mais mêmes coordonnées : portignagne ; RNN_bagnas ; B100 
...
**Interpretation**
- butor : observations depuis le début, depuis ~2018 plus vraiment d'observations. 
          observations les premières semaines de l'années et dernières semaines 
          sorties sans observations à partir de ~2013. (si on se refère aux graphs protocoles on voit que ça suit le même pattern. est ce que protocole fait à un moment étrange ?) 
          les periodes où observations ne sont plus faites même en non observation

- blongios : même pattern de semaines depuis ~1998 . qq sorties sans observation 
             à l'air de coller avec les patterns de protocoles 
...
sans ADENA : obs commencent à 2011/13? 
obs principaux : Mathier Lognos (jusqu'à 2022?) et Nathalie Guenel
...
Protocole_Scientifique <- process_data %>%
  filter( champs_additionnels %in% c("{'RELV_NOM': 'SuiviButor'}", "{'RELV_NOM': 'SuiviBlongiostaleve'}"))
...
Protocole_SuiviBlongiostaleve<- Protocole_Scientifique %>%
  filter( champs_additionnels == "{'RELV_NOM': 'SuiviBlongiostaleve'}")
