...
Ajout des variables climatiques une par une pour voir si elles ont un effet significatif sur l'abondance du Blongios nain.

```{r}

mod_pois <- glmmTMB(
  nombre_min ~ scale(annee, center = TRUE, scale = FALSE),
  family = poisson,
  data = Protocole_SuiviBlongios
)

disp <- sum(residuals(mod_pois, type = "pearson")^2) / df.residual(mod_pois)
disp
...
summary(ModeleAbondanceAnneePluie)
summary(ModeleAbondanceAnneeTemperature)
summary(ModeleAbondanceAnneeProfondeur)
summary(ModeleAbondanceAnneeAssec)

summary(ModeleAbondanceAnneeSite)

```

on voit que la pluie effet légèrement significatif
...
levels(Protocole_SuiviBlongiosGraph$nom_lieu)
...
emm_site <- emmeans(
  ModeleAbondanceAnneeSite,
  ~ nom_lieu,
  type = "response"   # résultats sur l’échelle de l’abondance
)
