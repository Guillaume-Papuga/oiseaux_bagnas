...
Par conséquent, nous avons utilisé un modèle ***logistf*** (régression logistique pénalisée de Firth) qui permet de fournir des estimations stables et des intervalles de confiance fiables lorsqu'il y a de très rares présences. Cependant, nous réduisons le nombre de variables explicatives et nous nous limitons à une estimation d'une tendance temporelle interanuelle (variable annee).

```{r}

## Modèle logistf avec uniquement l'effet de l'année
ModeleOccurenceLogistf <- logistf(Occurence ~ annee, data = Protocole_SuiviBlongios)
summary(ModeleOccurenceLogistf)
...
ggplot(annees, aes(x = annee)) +
  ...
  labs(
    ...
    title = "Probabilité prédite de présence en fonction des années",
    subtitle = "Bande = IC à 95 %"
  ) +
  theme_classic(base_size = 14)

```

L'effet de l'année sur la probabilité de présence du Blongios est négatif mais non significatif (β = `, round(coef(ModeleOccurenceLogistf)["annee"], 3),`, `p = ", round(summary(ModeleOccurenceLogistf)$coefficients["annee", "p"], 3),`). Cela signifie qu'il n'y a pas de tendance prononcée au cours du temps. On peut toutefois peut quand même relever qu'en visualisant l'effet graphiquement, on observe une tendance légèrement négative de la probablité de présence du Blongios nain sur la réserve.
...
On observe un nombre important de zéros dans la variable réallons essayer ponse abondance (nombre d'individus observés). Par conséquent, nous avons choisi d'utiliser un modèle GLM à inflation de zéros (glmmTMB). Et similairement au modèle de présence, nous allons seulement essayer de détecter une tendance temporelle, car l'estimation de trop de paramètres peut être instable avec un si petit jeu de données.
