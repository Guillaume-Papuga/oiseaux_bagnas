...
DataHydroBagnas       <- read_csv("data/pro/DataHydroBagnas.csv", show_col_types = FALSE)
environnement_daily   <- read_csv("data/pro/environnement_daily.csv", show_col_types = FALSE)
```
...
data_taleve_summarized <- data_taleve %>%
  ...
  summarise(
    statut = case_when(
      any(presence & is_protocole) & any(presence & !is_protocole) ~ "obs_mixte",
      any(presence & is_protocole)                                ~ "obs_protocole",
      any(presence & !is_protocole)                               ~ "obs_hors_protocole",
      any(is_protocole)                                           ~ "sortie_sans_observation",
      TRUE                                                        ~ NA_character_
    ),
    .groups = "drop"
  )
...
grille <- expand.grid(
  ...
  mutate(
    statut = replace_na(statut, "pas_de_sortie"),
    statut = factor(
      statut,
      levels = c(
        "obs_protocole",
...
#préparation du jeu de données pour le barplot
data_plot_horaire_protocole_Taleve <- heures_jour %>%
  left_join(
    Journalier_Protocole_Taleve %>%
      group_by(heure_tronc) %>%
      summarise(
        effort    = n(),
        n_taleves = sum(nombre_min, na.rm = TRUE),
        .groups = "drop"
      ),
    by = "heure_tronc"
  ) %>%
...
# graphique Effort d'échantillonnage et observations par heure\nProtocole Talève
...
Opportuniste_Taleve <-data_taleve %>%
  filter(
    !grepl("SuiviBlongiosTalève|SuiviButor", champs_additionnels)
  ) %>%
  mutate(
...
data_plot_horaire_opportuniste_Taleve <- heures_jour %>%
  left_join(
    Opportuniste_Taleve %>%
      group_by(heure_tronc) %>%
      summarise(
...
Voir ou mettre le texte suivant en debut en 1. ou ICI
...
# Modele null = sans variables explicatives 

modele_null <- glm(Occurence~ annee + julian + I(julian^2)+ nom_lieu ,
                   data = Protocole_SuiviTaleve, 
                   family = binomial )

# Modele complet = avec les variables explicatives qui nous semblent interresantes pour expliquer la présence de la Talève 
