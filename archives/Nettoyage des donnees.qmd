...
```{r}
#Nettoyage du tableau environnemental
...
environnement_daily <- environnement_daily %>%
  arrange(Date) %>%   # recommandé
  mutate(
    moy_temp_semaine = rollmean(temperature, k = 7, fill = NA, align = "right"),
    moy_temp_mois    = rollmean(temperature, k = 30, fill = NA, align = "right")
  ) %>%
  select(Date, moy_temp_semaine, moy_temp_mois) %>%
...
process_protocole_bis <- process_protocole %>%
  left_join(
    environnement_daily %>%
      select(Date, moy_temp_semaine, moy_temp_mois),
    by = c("date_debut" = "Date")
  )
```
