...
tableau_addition_site <- Protocole_SuiviTaleve %>%
  group_by(date_debut, temp_moy_glissante_7j, pluie_1h, pluie_moy_glissante_7j, pluie_moy_glissante_31j,AssecT_1, Profondeur) %>%
  summarise(
    total_obs = sum(nombre_min, na.rm = TRUE),
    .groups = "drop"
  )
...
tableau_ready<- tableau_repet %>%
  select(annee, passage, total_obs,temp_moy_glissante_7j, pluie_1h, pluie_moy_glissante_7j, pluie_moy_glissante_31j, AssecT_1, Profondeur) %>%
  pivot_wider(
    names_from = passage,
    values_from = total_obs,
    names_prefix = "passage_"
...
obscov<-list(temp=temp,pluie_1h=pluie_1h,pluie_7j=pluie_7j,pluie_31j=pluie_31j,profondeur=profondeur, assec=assec )	

	

#On refait le jeu de données.
taleve_nmixture<-unmarkedFramePCount(comptage,obsCovs=obscov)
...
backTransform(T_count_pluie1h, "state")
backTransform(T_count_pluie1h, "det")

predict(T_count_pluie1h, type="state")
predict(T_count_pluie1h, type="det")
