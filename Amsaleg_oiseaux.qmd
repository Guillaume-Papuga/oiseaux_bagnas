...
library(readr) 
DataHydroBagnas <- read_csv("data/pro/DataHydroBagnas.csv") 
View(DataHydroBagnas)
...
création des données pour N-Mixture 
```{r}
...
```{r}
Protocole_SuiviBlongios2 <- Protocole_SuiviBlongios %>%
  arrange(annee, nom_lieu, julian) %>%
  group_by(annee,nom_lieu) %>%
  mutate(visite = row_number()) %>%
  ungroup()


max(Protocole_SuiviBlongios2$visite)

Z <- Protocole_SuiviBlongios2 %>%
  pivot_wider(
    id_cols = c(nom_lieu, annee),
    names_from = visite,
...
T_count_List <- fitList('{lambda, p}' = T_count_null, 
                  ...
                  '{lambda, assec}' = T_count_assec)

modSel(T_count_List)


predict(T_count_temp, type = 'det')
